...
import { apiKeyRepo } from '../../db/repositories/index.js';
...
export async function loadDefaultSystemApiKey() {
    ...
    let apiKey = await apiKeyRepo.findSystemDefault({
        name: defaultName,
        encryptedKey: systemApiKey,
    });
    if (!apiKey) {
        // In-memory placeholder when no system key is persisted.
        // Mirrors previous behavior of returning an unsaved Mongoose doc.
        apiKey = {
            id: null,
            ownerId: null,
            provider: 'ollama',
 ...
 *
 * @param {object} user - User document with id
...
export async function loadProvider(user, options = {}) {
    ...
    let apiKeyDoc = await apiKeyRepo.findFirstForOwner(user.id);
    if (!apiKeyDoc) {
