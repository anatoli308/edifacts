...
export class Planner extends EventEmitter {
    ...
    _buildUserPrompt(userMessage, executionContext) {
        ...
        //TODO: anatoli build executionContext info into prompt
        /*
        const fileInfo = executionContext.fileInfo ? `\n\nFile: ${executionContext.fileInfo}` : '';
        // Add context information
        if (executionContext.analysisChat) {
            prompt += `\n**Context:** User has EDIFACT analysis data available:`;
            prompt += `\n- Segments: ${executionContext.analysisChat?.domainContext?.edifact?._analysis?.segments || 0}`;
            prompt += `\n- Errors: ${executionContext.analysisChat?.domainContext?.edifact?._analysis?.validation.errors.length || 0}`;
            prompt += `\n- Message type: ${executionContext.analysisChat.domainContext?.edifact?._analysis?.detectedMessageType || 'Unknown'}`;
        }
        prompt += fileInfo;
        */
