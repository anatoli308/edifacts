...
export class AnthropicAdapter {
    ...
    constructor(config) {
        ...
        this.capabilities = {
            ...
            context_window: modelEntry?.contextWindow ?? 200000,
        };
    }
    ...
    async *streamComplete({ messages, tools, options = {} }) {
        try {
            ...
            let accumulatedUsage = null;  // ✨ Track usage
            let currentToolUse = null;
            
            for await (const event of stream) {
                // Track usage from message_start (initial usage)
                if (event.type === 'message_start' && event.message?.usage) {
                    accumulatedUsage = event.message.usage;
                }
                ...
                // ✨ Track usage from message_delta event (incremental usage)
                if (event.type === 'message_delta' && event.usage) {
                    // Merge with existing usage (Anthropic sends incremental usage)
                    if (accumulatedUsage) {
                        accumulatedUsage.output_tokens = event.usage.output_tokens || accumulatedUsage.output_tokens;
                    } else {
                ...
                if (event.type === 'message_stop') {
                    // Debug: Check if usage is present
                    if (!accumulatedUsage) {
                        console.warn('[AnthropicAdapter] No usage data collected from stream.');
                    }
                    ...
                    yield {
                        ...
                        usage: accumulatedUsage || null  // ✨ Include usage (may be null)
                    };
                }
            }
