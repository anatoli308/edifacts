 ...
 * - description: required, string, 10-1024 chars (OpenAI/Anthropic upper bound)
...
export function validateToolDefinition(tool) {
    ...
    } else if (tool.description.length < 10 || tool.description.length > 1024) {
        errors.push('description must be 10-1024 characters');
    }
...
export function validateToolArguments(arguments_, schema) {
    ...
    // JSON Schema distinguishes `integer` from `number`, but JS `typeof` only knows `number`.
    // Treat `integer` as: typeof === 'number' AND Number.isInteger(value).
    if (schema.properties && typeof schema.properties === 'object') {
            ...
            if (field in arguments_) {
                ...
                if (fieldSchema.type && value !== null) {
                    ...
                    if (expectedType === 'integer') {
                        typeOk = actualType === 'number' && Number.isInteger(value);
                    } else if (expectedType === 'number') {
                        typeOk = actualType === 'number';
                    } else {
