...
export class Planner {
     ...
     * @param {object} params.context - agent context (sessionId, userId, socket, etc)
     * @param {object} params.provider - LLM provider (required)
     * @returns {Promise<object>} Task plan with subtasks and execution order
     */
    async invoke({ userMessage, messages, context = {}, provider = null }) {
        ...
        try {
            ...
            const llmResult = await this._decomposeWithLLM(userMessage, messages, context, provider);
            ...
            return this._finalizeResult(taskPlan, userMessage, 'LLM', startTime);
        } catch (error) {
     ...
     * @private
     */
    async _decomposeWithLLM(userMessage, messages, context, provider) {
        let lastError = null;
        ...
        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                console.log('[Planner] Successfully parsed and streaming plan to client', parsed.subtasks.length, 'subtasks');
                ...
                return { subtasks: parsed.subtasks, rationale: parsed.rationale };
            } catch (error) {
                lastError = error;
