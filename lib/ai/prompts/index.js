 ...
 *
 * Exports:
 * - System prompts for each agent (Planner, Executor, Critic)
 ...
 * @param {string} agent - Agent name (planner, executor, critic, assistant)
...
export const getPrompt = (agent) => {
  ...
  if (!prompt) {
    throw new Error(`Unknown agent: ${agent}. Available: planner, executor, critic, assistant`);
  }
  return prompt;
};
