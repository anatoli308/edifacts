...
function handleAgentInvoke(socket, user) {
    return async (request) => {
        ...
        try {
            ...
            // Prepare messages for agent: use context messages if available, otherwise create new
            let messages;
            if (context?.latestMessages && Array.isArray(context.latestMessages)) {
                console.log(`[Agent Handler] Context messages received: ${context.latestMessages.length} total`);
                // Filter context messages to avoid token explosion
                messages = filterContextMessages(context.latestMessages, 5);
                console.log(`[Agent Handler] Filtered messages: ${messages.length} (max 5, excluding tool-only)`);
                console.log('[Agent Handler] Message roles:', messages.map(m => {
                    const contentStr = typeof m.content === 'string'
                        ? m.content
                        : (m.content?.text || JSON.stringify(m.content || '')).substring(0, 50);
                ...
                if (!lastMessage || lastMessage.role !== 'user' || lastMessage.content !== userMessage) {
                    ...
                    console.log(`[Agent Handler] Added current user message: "${userMessage.substring(0, 50)}..."`);
                }
            } else {
                // No context messages: start fresh
                console.log('[Agent Handler] No context messages - starting fresh conversation');
                messages = [
                    {
                        role: 'user',
                        content: userMessage,
            ...
            console.log(`[Agent Handler] Final message count for agent: ${messages.length}`);

            // Invoke agent
            const agentContext = {
                ...context,
                messages, // Pass filtered conversation history
                sessionId,
                userId: user._id,
            ...
            if (agentType.toLowerCase() === 'router' && agentResult?.assistantMessage) {
                ...
                // For FULL_PIPELINE: Skip final response chunk (Executor already streamed it)
                // For FAST_PATH: Skip final response chunk (Router already streamed it)
                // Streaming happens in real-time during agent execution
            } // End of if (Router)

            // Stream agent result (for non-Router agents, Planner, Executor, etc)
            else if (agentResult?.taskTree) {
            ...
            if (agentType.toLowerCase() !== 'router' && agentResult?.toolCalls?.length > 0) {
