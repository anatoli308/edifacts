 ...
 * Human Analog: Anterior Cingulate Cortex (Error Detection, Quality Control)
 * 
 ...
 * - Comprehensive: catch issues before next task
...
export class Critic extends EventEmitter {
    /**
    ...
    /**
     * Reset critic state for next validation
     */
    reset() {
        console.log('[Critic] State reset');
        // Hook for future state cleanup (validation cache, etc.)
    }
     ...
     */
    async invoke({ output, validators = {}, context = {}, provider = null }) {
        this.emit('agent_critic:started', { timestamp: Date.now() });
        ...
        try {
            ...
            //TODO: better security checks anatoli
            ...
            if (validators.factChecker) {
                ...
                if (hallucinations.length > 0) {
                    ...
                    // Separate error-level hallucinations (metric mismatches) from warnings
                    const errorHallucinations = hallucinations.filter(h => h.severity === 'error');
                    const warningHallucinations = hallucinations.filter(h => h.severity !== 'error');

                    if (errorHallucinations.length > 0) {
                        result.errors.push({
                            type: 'hallucination_error',
                            message: `${errorHallucinations.length} metric mismatch(es): ${errorHallucinations.map(h => h.claim).join('; ')}`,
                            severity: 'error'
                        });
                    }

                    if (warningHallucinations.length > 0) {
                        result.warnings.push({
                            type: 'hallucination_detected',
                            message: `${warningHallucinations.length} potential hallucination(s) detected`,
                            severity: 'warning'
                        });
                    }

                    result.valid = false;
                    result.reasoning.push(`Hallucinations detected: ${hallucinations.length} (${errorHallucinations.length} errors, ${warningHallucinations.length} warnings)`);
                }
            }
    ...
    _finalizeResult(result, startTime) {
        ...
        this.emit('agent_critic:completed', {
            valid: result.valid,
            score: result.score,
