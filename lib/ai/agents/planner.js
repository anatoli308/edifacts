 ...
 * - Domain context (EDIFACT data, file, rules, available tools)
...
export class Planner {
     ...
     * @param {object} params.context - Domain context (EDIFACT data, history, available tools)
    ...
    async invoke({ goal, context = {}, provider = null }) {
        ...
        try {
            ...
            const llmResult = await this._decomposeWithLLM(goal, context, provider);
    ...
    /**
     * LLM-based decomposition - intelligently decompose goal into subtasks
     * @private
     */
    async _decomposeWithLLM(goal, context, provider) {
        try {
            ...
            console.log('[Planner._decomposeWithLLM] Calling LLM for task decomposition');
            ...
            console.log('[Planner._decomposeWithLLM] LLM response received');
            ...
            console.log('[Planner._decomposeWithLLM] Successfully parsed', parsed.subtasks.length, 'subtasks');

            return {
                subtasks: parsed.subtasks,
                rationale: parsed.rationale || 'LLM-based task decomposition'
            };
        } catch (error) {
            console.error('[Planner._decomposeWithLLM] Error:', error);
    ...
    _buildUserPrompt(goal, context) {
        ...
        prompt += `\n\n**Requirements:**`;
        prompt += `\n- Break down the goal into 2-6 concrete, executable subtasks`;
