 ...
 *
 * Responsibilities:
 * - Receive a goal from Router Agent and agent context
 ...
 * - agent context (EDIFACT data, sessionId, userId, socket, etc)
...
import { PLANNER_CONFIG } from '../config/index.js';
...
export class Planner {
     ...
     * @param {object} params.context - agent context (EDIFACT data, sessionId, userId, socket, etc)
     * @param {object} params.provider - LLM provider (required)
     * @returns {Promise<object>} Task plan with subtasks and execution order
     */
    async invoke({ goal, messages, context = {}, provider = null }) {
        const startTime = Date.now();

        try {
            // Validate provider
            if (!provider) {
                throw new Error('Provider is required for Planner agent');
            }
            ...
            console.log('[Planner] Decomposing goal:', goal.substring(0, 100));

            // Use LLM to decompose goal into task plan
            const llmResult = await this._decomposeWithLLM(goal, messages, context, provider);

            // Build task graph with dependencies and execution order
            const taskPlan = this._buildTaskGraph(llmResult.subtasks, goal, llmResult.rationale);

            // Finalize result with metadata
            return this._finalizeResult(taskPlan, goal, 'LLM', startTime);
        } catch (error) {
     ...
     * @private
     */
    //TODO: messages hinzu anatoli
    async _decomposeWithLLM(goal, messages, context, provider) {
        let lastError = null;
        const socket = context.socket; // Extract socket for streaming

        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            try {
                const systemPrompt = this._buildSystemPrompt(context);
                const userPrompt = this._buildUserPrompt(goal, context);

                console.log(`[Planner] Decomposition attempt ${attempt + 1}/${this.config.maxRetries + 1}`);

                // Get available tools from registry (if context provides module or tool names)
                const availableTools = await this._getAvailableTools(context);
                ...
                console.log('[Planner] Successfully parsed', parsed.subtasks.length, 'subtasks');

                // Stream plan immediately to client
                if (socket && typeof socket.emit === 'function') {
                    console.log('[Planner] Streaming plan to client (', parsed.subtasks.length, 'subtasks)');
                    socket.emit('agent:plan', {
                        subtasks: parsed.subtasks,
                        rationale: parsed.rationale || 'LLM decomposition',
                        goal: goal,
                        timestamp: Date.now()
                    });
                }

                return { subtasks: parsed.subtasks, rationale: parsed.rationale || 'LLM decomposition' };
            } catch (error) {
                lastError = error;
     ...
     * @private
     */
    async _getAvailableTools(context) {
        ...
        try {
            ...
            for (const tool of allTools) {
                if (!context.module || tool.module === context.module) {
                    tools.push({
                        name: tool.name,
    ...
    _buildSystemPrompt(context) {
        const basePrompt = getPrompt('planner');
        const fileInfo = context.fileInfo ? `\n\nFile: ${context.fileInfo}` : '';
        return basePrompt + fileInfo;
    }
    ...
    _buildUserPrompt(goal, context) {
        ...
        if (context.analysis) {
            ...
            prompt += `\n- Segments: ${context.analysis.totalSegments || 0}`;
            prompt += `\n- Errors: ${context.analysis.errors?.length || 0}`;
            prompt += `\n- Message type: ${context.analysis.messageType || 'Unknown'}`;
        }
        ...
        try {
            ...
            if (!jsonStr) {
                ...
                if (anyJsonMatch) {
                    ...
                    console.log('[Planner._parseLJMResponse] Using fallback JSON extraction');
                }
            }
