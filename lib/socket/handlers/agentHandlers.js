    ...
    return async (request) => {
        ...
        try {
            ...
            if (messages && Array.isArray(messages) && messages.length > 0) {
                console.log(`[Agent Handler] Messages received: ${messages.length} total`);
                // Filter messages to avoid token explosion (max 5 turns)
                conversationMessages = filterContextMessages(messages, 5);
                console.log(`[Agent Handler] Filtered messages: ${conversationMessages.length} (max 5, excluding tool-only)`);
            } else {
            ...
            console.log(`[Agent Handler] Final message count for agent: ${conversationMessages.length}`);
