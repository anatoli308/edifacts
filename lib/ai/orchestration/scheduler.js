...
export class Scheduler extends EventEmitter {
        ...
        try {
            ...
            for (const taskId of executionOrder) {
                ...
                try {
                    ...
                    // Step 4b: Check if Executor reported incomplete execution (e.g. uncalled mandatory tools)
    ...
    async _executeTask(task, agents, executionContext, messages, dependencyResults = {}, isLastTask = false) {
        ...
        try {
            ...
            const edifactToolNames = [
                'segmentAnalyze', 'groupSegmentsByType', 'parseSegmentField',
                'compareSegments', 'validateRules', 'checkCompliance',
                'detectAnomalies', 'validateDataTypes', 'suggestFixes'
            ];
            const taskNeedsRawEdifact = task.tools && task.tools.some(t => edifactToolNames.includes(t));
     ...
     * @private
     */
    async _validateTask(taskResult, agents, task, executionContext) {
        if (!agents.critic) {
        ...
        try {
            ...
            // and the task involves EDIFACT tools
            const edifactToolNames = [
                'segmentAnalyze', 'groupSegmentsByType', 'parseSegmentField',
                'compareSegments', 'validateRules', 'checkCompliance',
                'detectAnomalies', 'validateDataTypes', 'suggestFixes'
            ];
            const taskHasEdifactTools = task.tools && task.tools.some(t => edifactToolNames.includes(t));
