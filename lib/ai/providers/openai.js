...
export class OpenAIAdapter {
     ...
     * @param {object} config.registry - Provider registry from providers.config.js (required)
     * @param {object} config.providerConfig - Provider-specific config from providers.config.js (required)
     * @throws {Error} If apiKey, registry, or providerConfig is missing
     */
    constructor(config) {
        if (!config?.apiKey) {
        ...
        if (!config?.registry) {
            throw new Error('Provider registry is required');
        }
        ...
        this.config = {
            ...
            maxTokens: 4096,
            requestTimeoutMs: config.providerConfig.timeout_ms,
            baseUrl: config.providerConfig.api_base,
            ...config
        };
        ...
        this.client = new OpenAI({
            ...
            retry_strategy: {
                maxRetries: 3,
                backoff: { type: config.providerConfig.retry_strategy, min: 1000, max: 10000 }
            }
        });

        this.name = config.registry.name.toLowerCase();

        const { features, models } = config.registry;
        const modelEntry = models.find(m => m.id === this.config.model);

        this.capabilities = {
            parallel_tool_calls: features.parallel_tool_calls,
            streaming: features.streaming,
            vision: features.vision,
            function_calling: features.function_calling,
            context_window: modelEntry?.contextWindow ?? 4096,
        };
    }
    ...
    async complete({ messages, tools, systemPrompt, options = {} }) {
        try {
            ...
            const response = await this.client.chat.completions.create({
                ...
                ...(convertedTools ? { tools: convertedTools, tool_choice: options.toolChoice || 'auto' } : {}),
            });
    ...
    async *streamComplete({ messages, tools, systemPrompt, options = {} }) {
        try {
            ...
            const stream = await this.client.chat.completions.create({
                ...
                ...(convertedTools ? { tools: convertedTools, tool_choice: options.toolChoice || 'auto' } : {}),
                stream: true,
                stream_options: { include_usage: true }  // Required to get usage in streaming
            });
            ...
            for await (const chunk of stream) {
                ...
                if (finishReason) {
                    ...
                    // Usage may arrive in this chunk or in a subsequent chunk with empty choices.
                    // Continue reading to capture the usage chunk if not present here.
                    let usageData = chunk.usage || null;

                    if (!usageData) {
                        // Try reading next chunk(s) for usage data (OpenAI sends usage in a separate final chunk)
                        try {
                            for await (const extraChunk of stream) {
                                if (extraChunk.usage) {
                                    usageData = extraChunk.usage;
                                    break;
                                }
                            }
                        } catch (_) {
                            // Stream may already be exhausted
                        }
                    }

                    if (!usageData) {
                        console.warn('[OpenAIAdapter] No usage data received from stream.');
                    }
