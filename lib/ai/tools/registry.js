...
class ToolRegistry {
  ...
  validate(name, args) {
    ...
    // TODO: Use JSON Schema validator (e.g., ajv)
    // For now, basic validation
    if (!args || typeof args !== 'object') {
      throw new Error(`Invalid arguments for tool ${name}: must be object`);
    }
    return true;
  }
  ...
  _validateTool(tool, name) {
    if (!tool.name) throw new Error(`Tool ${name} missing 'name'`);
