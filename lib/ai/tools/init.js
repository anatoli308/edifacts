...
export async function initializeToolRegistry() {
  ...
  try {
    ...
    try {
      const utilityModule = await import('../../../_modules/utility/tools/index.js');
      if (utilityModule.tools) {
        registry.register(utilityModule.tools, 'utility');
        registeredModules.push('utility');
        console.log(`✓ Registered Utility tools (${Object.keys(utilityModule.tools).length} tools)`);
      }
    } catch (err) {
