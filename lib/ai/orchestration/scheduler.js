...
export class Scheduler extends EventEmitter {
    ...
    async execute({ taskTree, agents, executionContext = {}, messages = [] }) {
        ...
        try {
            ...
            // Step 3: Execute tasks dynamically — after each successful task we ask
            // the LLM whether to continue, stop early, or modify the remaining plan.
            const completedTasks = new Set();
            const remainingOrder = [...executionOrder];
            const totalPlanned = depGraph.tasks.length;
            let stepCount = 0;
            let stoppedEarly = false;

            while (remainingOrder.length > 0) {
                if (stepCount >= this.config.maxSteps) {
                    console.warn(`[Scheduler] Max steps (${this.config.maxSteps}) reached, stopping`);
                    this.emit('agent_scheduler:max_steps_reached', {
                        stepCount,
                        maxSteps: this.config.maxSteps,
                        remainingTasks: remainingOrder.length,
                        timestamp: Date.now()
                    });
                    break;
                }

                const taskId = remainingOrder.shift();
                stepCount++;
                ...
                // Dynamic progress: total may grow if continuation adds tasks
                const progressTotal = Math.max(totalPlanned, stepCount + remainingOrder.length);

                // Stream task start to client
                console.log(`[Scheduler] Emitting task_started: ${task.name} (${stepCount}/${progressTotal})`);
                ...
                try {
                    // Last task = no remaining tasks queued. Continuation may still add
                    // more, but we mark it last for streaming purposes.
                    const isLastTask = remainingOrder.length === 0;
                    ...
                    if (!taskResult.success && taskResult.uncalledTools?.length > 0) {
                        ...
                        this.emit('agent_scheduler:step', {
                            ...
                            progress: {
                                ...
                                total: Math.max(totalPlanned, stepCount + remainingOrder.length)
                            },
                            timestamp: Date.now()
                        });
                    ...
                    if (!validation.valid) {
                        ...
                        if (taskResult.assistantMessage && taskResult.assistantMessage.length > 0) {
                            ...
                            console.log(`[Scheduler] Emitting task_completed (with warnings): ${task.name} (${completedTasks.size}/${Math.max(totalPlanned, stepCount + remainingOrder.length)})`);
                            this.emit('agent_scheduler:step', {
                                step: 'task_completed',
                                taskId: taskId,
                                taskName: task.name,
                        ...
                        console.log(`[Scheduler] Emitting task_completed: ${task.name} (${completedTasks.size}/${Math.max(totalPlanned, stepCount + remainingOrder.length)})`);
                        this.emit('agent_scheduler:step', {
                            step: 'task_completed',
                            taskId: taskId,
                            taskName: task.name,
                ...
                // Step 6: After each successful task, ask the Decision agent
                // whether to continue, stop early, or modify the remaining plan.
                // We only invoke when there's still work pending and a decision
                // agent has been injected.
                if (completedTasks.has(taskId) && remainingOrder.length > 0 && this.decision) {
                    try {
                        const decision = await this.decision.invoke({
                            goal: taskTree.goal,
                            completedTasks,
                            subtaskResults: result.subtaskResults,
                            remainingTasks: remainingOrder.map(id => depGraph.taskMap.get(id)).filter(Boolean),
                            executionContext
                        });
                        ...
                        this.emit('agent_scheduler:step', {
                            ...
                            addedTasks: (decision.addTasks || []).map(t => ({ id: t.id, name: t.name })),
                            timestamp: Date.now()
                        });

                        if (decision.action === 'stop') {
                            console.log(`[Scheduler] Stopping early after step ${stepCount}: ${decision.reason}`);
                            stoppedEarly = true;
                            break;
                        }

                        if (decision.action === 'modify') {
                            // Remove tasks marked as no longer needed
                            const removeIds = Array.isArray(decision.removeIds) ? decision.removeIds : [];
                            for (const id of removeIds) {
                                const idx = remainingOrder.indexOf(id);
                                if (idx !== -1) {
                                    remainingOrder.splice(idx, 1);
                                    console.log(`[Scheduler] Continuation removed task: ${id}`);
                                }
                            }

                            // Append newly added tasks
                            const addTasks = Array.isArray(decision.addTasks) ? decision.addTasks : [];
                            for (const newTask of addTasks) {
                                if (!newTask?.id || !newTask?.name) continue;
                                if (depGraph.taskMap.has(newTask.id)) {
                                    console.warn(`[Scheduler] Continuation tried to add duplicate task id: ${newTask.id}, skipping`);
                                    continue;
                                }
                                const normalized = {
                                    id: newTask.id,
                                    ...
                                    tools: Array.isArray(newTask.tools) ? newTask.tools : [],
                                    dependencies: [],
                                    effort: newTask.effort || 'LOW',
                                    dynamic: true
                                };
                                depGraph.taskMap.set(normalized.id, normalized);
                                depGraph.dependencies.set(normalized.id, []);
                                depGraph.tasks.push(normalized);
                                remainingOrder.push(normalized.id);
                                console.log(`[Scheduler] Continuation added task: ${normalized.id} (${normalized.name})`);
                            }
                        }
                    } catch (contErr) {
                        // Continuation failure is non-fatal — fall back to executing as planned
                        console.warn(`[Scheduler] Continuation decision failed, continuing with plan: ${contErr.message}`);
                    }
                }
            }

            if (stoppedEarly) {
                console.log(`[Scheduler] Pipeline stopped early; ${remainingOrder.length} planned task(s) skipped`);
            }

            // Step 7: Determine if goal is completed
