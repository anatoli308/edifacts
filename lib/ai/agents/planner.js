...
export class Planner {
    ...
    _decomposeWithHeuristics(goalType, message) {
        ...
        const confidence = Math.min(1.0, matchCount / Math.max(1, keywords.length * 0.5));
    ...
    async _decomposeWithLLM(goal, context, provider, heuristicHint) {
        try {
            ...
            const response = await Promise.race([
                provider.complete({ messages, system: systemPrompt, temperature: this.config.temperature }),
