...
export class Executor {
    constructor(config = {}) {
        this.config = {
            ...
            maxTokens: 4000,           // Max tokens for reasoning
            maxIterations: 10,         // Max ReAct iterations to prevent infinite loops
            iterationTimeoutMs: 5000,  // Timeout per iteration
            ...config
        };
    }
    ...
    async _callLLMForToolSelection(messages, availableTools, provider, context) {
        ...
        try {
            ...
            if (canStream) {
                console.log(`[Executor._callLJMForToolSelection] Streaming with ${universalTools.length} tools`);
