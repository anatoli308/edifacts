  ...
  register(toolsObject, module) {
    ...
    for (const [name, tool] of Object.entries(toolsObject)) {
      try {
        ...
        // Fail loud per tool, but do NOT abort registration of remaining tools.
        // Silent partial registration previously caused tools to "disappear"
        // and planners to hallucinate tool names that were never registered.
        failed.push({ name, error: err.message });
        console.error(`[ToolRegistry] Failed to register tool "${name}" (module=${module}): ${err.message}`);
      }
    }

    if (failed.length > 0) {
      console.error(
        `[ToolRegistry] ${failed.length} tool(s) in module "${module}" failed validation: ${failed.map(f => f.name).join(', ')}`
      );
    }
  }
