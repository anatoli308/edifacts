...
export function validateToolDefinition(tool) {
    ...
    } else if (!/^[a-z0-9_]+$/.test(tool.name)) {
        errors.push('name must be lowercase alphanumeric with underscores');
...
export function validateToolCall(call) {
    ...
    } else if (!/^[a-z0-9_]+$/.test(call.tool)) {
        errors.push('tool must be lowercase alphanumeric with underscores');
    }
