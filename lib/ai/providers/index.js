 ...
 */
export async function loadDefaultSystemApiKeyOpenAI() {
    //load OPENAI_API_KEY from env or create a default one
    const openaiApiKey = process.env.SYSTEM_API_KEY;
    const defaultName = 'Default System OpenAI Key';
    let apiKey = await ApiKey.findOne({ name: defaultName, encryptedKey: openaiApiKey, ownerId: null });
    if (!apiKey) {
        apiKey = new ApiKey({
            provider: 'openai',
            name: defaultName,
