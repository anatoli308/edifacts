...
const EXECUTOR_CONFIG = {
    ...
    maxCallsPerTool: 3, // Hard cap per tool name within one task to break runaway loops
    iterationTimeoutMs: 60000, // Per iteration safety net (LLM calls take 5-10s each)
};
...
export class Executor extends EventEmitter {
    ...
    async invoke({ messages, context = {}, toolNames }) {
        ...
        try {
            ...
            if (availableTools.length === 0) {
                ...
                // Authoritative directive: this task is synthesis-only. Contradicts any
                // earlier prompt that mentions tool catalogs or available tools — recency
                // wins, and tools=[] is also passed to the API so any tool_call attempt
                // is a contract violation that will be retried.
                const synthesisDirective = {
                    role: 'system',
                    content:
                        `SYNTHESIS-ONLY TASK (authoritative, overrides any earlier prompt sections about tools).\n` +
                        `No tools are available for this step. Do NOT emit a tool_call / function_call under any circumstance.\n` +
                        `Your job is to take the tool results already gathered in earlier tasks (visible in the conversation above) and produce the final user-facing Markdown answer.\n` +
                        `Start the answer immediately. Do not write meta-commentary like "I will now answer" or "We need to call the tool". Just write the answer.`
                };
                const messagesWithCatalog = [...messages, synthesisDirective];
                ...
                // Guard A: LLM emitted a tool_call even though tools=[] was passed.
                // In pure-text mode ANY tool call is a contract violation — content may
                // be empty ("") or a meta-comment ("We need to call the tool."), neither
                // is a real user-facing answer. Retry with explicit correction.
                const illegalToolCall =
                    Array.isArray(llmResponse.toolCalls) &&
                    llmResponse.toolCalls.length > 0;
                if (illegalToolCall) {
                    console.warn('[Executor.invoke] LLM emitted tool_call despite tools=[]. Retrying with correction.', {
                        attemptedTools: llmResponse.toolCalls.map(tc => tc.tool || tc.name),
                        priorContentLength: content?.length ?? 0
                    });
                    const correctionMessages = [
                        ...messagesWithCatalog,
                        { role: 'user', content: 'No tools are available for this task. Do NOT attempt any tool/function call. Synthesize the previous tool results from earlier tasks into a clear, user-facing Markdown answer. Output prose only — start writing the answer immediately, no meta-commentary about tools.' }
                    ];
                    const retry = await this._callLLMForToolSelection(correctionMessages, [], context);
                    content = retry.content;
                }

                // Guard B: Detect if LLM output a raw JSON tool call instead of prose
                if (content && this._looksLikeRawToolCall(content)) {
                    console.warn('[Executor.invoke] LLM output raw JSON tool call as text. Retrying with correction.');
                    const correctionMessages = [
                        ...messagesWithCatalog,
                        { role: 'assistant', content },
                        { role: 'user', content: 'Your previous output was raw JSON, not a user-facing response. This task requires a formatted Markdown report synthesizing the previous tool results. Do NOT output JSON. Write a clear, structured analysis report for the user.' }
                    ];
                    const retry = await this._callLLMForToolSelection(correctionMessages, [], context);
                    content = retry.content;
                }
            ...
            // Determine mandatory tools from the task plan
            const mandatoryTools = context.currentTask?.tools || [];
            ...
            while (iteration < this.config.maxIterations) {
                ...
                try {
                    ...
                    if (!llmResponse.toolCalls || llmResponse.toolCalls.length === 0) {
                        // Before accepting TASK_COMPLETE, verify all mandatory tools were called
                        const uncalledTools = mandatoryTools.filter(t => !calledToolNames.has(t));

                        if (uncalledTools.length > 0) {
                            // LLM tried to stop early — force it to call remaining tools
                            console.warn(`[Executor.invoke] LLM tried to stop but ${uncalledTools.length} mandatory tools not called: ${uncalledTools.join(', ')}`);
                            ...
                            currentMessages.push({
                                role: 'user',
                                content: `You have NOT yet called these MANDATORY tools: ${uncalledTools.join(', ')}. `
                                    + 'You MUST call each of them now. Do NOT finish without calling every assigned tool. '
                                    + 'Call the remaining tools with the raw EDIFACT data.'
                            });

                            executionTrace.push({
                                iteration,
                                step: 'MANDATORY_TOOL_ENFORCEMENT',
                                uncalledTools,
                                reasoning: 'LLM attempted to stop before calling all mandatory tools'
                            });
                            continue; // Re-enter the loop
                        }

                        // All mandatory tools called — task is genuinely complete
                        executionTrace.push({
                            iteration,
                            step: 'THOUGHT',
                    ...
                    for (const toolCall of llmResponse.toolCalls) {
                        ...
                        // Track mandatory tool completion
                        calledToolNames.add(toolCall.tool);

                        // Hard cap: short-circuit further calls of the same tool with a
                        // synthetic error result so the LLM gets explicit feedback in the
                        // next iteration instead of looping silently.
                        if (callCount > this.config.maxCallsPerTool) {
                            console.warn(`[Executor.invoke] \u26a0\ufe0f Tool "${toolCall.tool}" exceeded maxCallsPerTool (${this.config.maxCallsPerTool}). Short-circuiting.`);

                            const cappedResult = {
                                id: toolCall.id,
                                tool: toolCall.tool,
                                success: false,
                                error: `Tool "${toolCall.tool}" has already been called ${callCount - 1} times in this task. Further calls are blocked. Use the previous results to answer the user.`,
                                result: null,
                                callId: toolCall.id,
                                duration_ms: 0
                            };
                            ...
                            iterationToolResults.push(cappedResult);
                            toolCalls.push(toolCall);
                            toolResults.push(cappedResult);
                            continue;
                        }

                        console.log(`[Executor.invoke] Executing tool: ${toolCall.tool} (call #${callCount})`, JSON.stringify(toolCall.arguments));
                        ...
                        if (toolResult.success) {
                            ...
                            const summary = r?.resultCount !== undefined
                                ? `mode=${r.mode} count=${r.resultCount} topSim=${r.results?.[0]?.similarity?.toFixed(3) ?? 'n/a'} topSource=${r.results?.[0]?.source ?? 'n/a'}`
                                : 'success';
                            console.log(`[Executor.invoke] Tool result: ${toolCall.tool} = ${summary}`);
                        } else {
                            console.warn(`[Executor.invoke] Tool result: ${toolCall.tool} = FAILED: ${toolResult.error || toolResult.result?.error || 'unknown'}`);
                        }
            ...
            // Check if all mandatory tools were actually called
            const uncalledMandatory = mandatoryTools.filter(t => !calledToolNames.has(t));
            const allMandatoryCalled = uncalledMandatory.length === 0;

            if (!allMandatoryCalled) {
                console.warn(`[Executor] Task ended with uncalled mandatory tools: ${uncalledMandatory.join(', ')}`);
            }

            const result = {
                success: allMandatoryCalled,
                toolCalls,
                toolResults,
                executionTrace,
                iterationCount: iteration,
                maxIterationsReached: iteration >= this.config.maxIterations,
                uncalledTools: uncalledMandatory,
            ...
            try {
                ...
                if (!context.isLastTask) {
                    messagesWithSystem.push({
                        role: 'system',
                        content: 'IMPORTANT: This is an INTERMEDIATE task in a multi-step pipeline. '
                            + 'Your output will be used as input for subsequent tasks, NOT shown directly to the user. '
                            + 'RULES: '
                            + '1. You MUST call ALL tools that are assigned to this task. Never skip a tool because previous results seem sufficient. Each tool provides unique analysis. '
                            + '2. Before each tool call, write 1-2 sentences explaining what you are about to analyze and why. This text is shown to the user as your reasoning. '
                            + '3. After receiving tool results, write 2-4 sentences summarizing key findings. '
                            + '4. Do NOT write a full user-facing answer or greeting. Focus on analytical notes.'
                    });
                }
    ...
    /**
     * Detect if LLM text output is a raw JSON tool call instead of a user-facing response.
     * This happens when the LLM is confused and outputs a function call as plain text.
     * @private
     */
    _looksLikeRawToolCall(content) {
        ...
        try {
            ...
            // Common tool call shapes: {name, arguments}, {tool, input}, {function, parameters}
            return (
                (typeof obj.name === 'string' && obj.arguments !== undefined) ||
                (typeof obj.tool === 'string' && obj.input !== undefined) ||
                (typeof obj.function === 'string' && obj.parameters !== undefined)
            );
        } catch {
            return false;
        }
