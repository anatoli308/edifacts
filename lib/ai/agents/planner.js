...
const PLANNER_CONFIG = {
    ...
    topP: 0.95,
    timeoutMs: 15000,
    maxRetries: 0, // TODO: for now no retries maybe later, but careful with replanning loops
...
export class Planner extends EventEmitter {
    ...
    reset() {
        ...
        // Hook for future state cleanup (planHistory, etc.)
    }
    ...
    async invoke({ userMessage, messages, executionContext = {} }) {
        ...
        try {
            ...
            console.log(`[Planner] Decomposing user message: ${userMessage.substring(0, 10)}...`);
            ...
            throw error;
            //TODO: anatoli - vielleicht wieder reinnehmen, mal schauen
            //return this._defaultResult(userMessage, error, startTime);
        }
    }

    /**
     * Replan after a task failure — creates a new plan incorporating feedback.
     * Called by Orchestrator when Scheduler signals needsReplanning.
     * 
     * @param {object} params
     * @param {object} params.originalPlan - The plan that failed
     * @param {object} params.replanFeedback - Feedback from scheduler (reason, failedTools, suggestion)
     * @param {object} params.executionContext - Execution context
     * @param {Array} params.messages - Chat conversation history
     * @returns {Promise<object>} New task plan
     */
    async replan({ originalPlan, replanFeedback, executionContext, messages = [] }) {
        const startTime = Date.now();

        console.log(`[Planner] Replanning triggered: ${replanFeedback.reason}`);

        this.emit('agent_planner:replanning', {
            reason: replanFeedback.reason,
            failedTaskId: replanFeedback.failedTaskId,
            suggestion: replanFeedback.suggestion,
            timestamp: Date.now()
        });

        try {
            // Build a replan prompt that incorporates the failure context
            const replanContext = [
                `REPLANNING REQUIRED - Previous plan failed.`,
                ``,
                `Original goal: ${originalPlan.goal}`,
                `Failed task: "${replanFeedback.failedTaskName}" (${replanFeedback.failedTaskId})`,
                `Failed tools: ${replanFeedback.failedTools?.join(', ') || 'unknown'}`,
                `Failure reason: ${replanFeedback.reason}`,
                `Suggestion: ${replanFeedback.suggestion}`,
                ``,
                `Create a NEW plan that avoids the failed approach.`,
                `Do NOT use the failed tools for this data. Use the suggested alternative instead.`
            ].join('\n');

            // Inject replan context into executionContext so _buildUserPrompt can use it
            const replanExecutionContext = {
                ...executionContext,
                _replanContext: replanContext
            };
            ...
            const taskPlan = this._buildTaskGraph(llmResult.subtasks, originalPlan.goal, llmResult.rationale);

            const finalResult = this._finalizeResult(taskPlan, originalPlan.goal, 'REPLAN', startTime);
            finalResult.metadata.replanning_count = (originalPlan.metadata?.replanning_count || 0) + 1;
            finalResult.metadata.replanReason = replanFeedback.reason;

            this.emit('agent_planner:completed', finalResult);

            console.log(`[Planner] Replan completed: ${finalResult.subtasks?.length || 0} subtasks (attempt ${finalResult.metadata.replanning_count})`);

            return finalResult;
        } catch (error) {
            console.error(`[Planner] Replan error: ${error.message}`);
            throw error;
        }
    }
        ...
        for (let attempt = 0; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                if (attempt === this.config.maxRetries || error.status === 429) {
                    const err = new Error(`Decomposition failed: ${lastError.message}`);
    ...
    _buildUserPrompt(userMessage, executionContext) {
        ...
        // Add replan context if this is a replanning call
        if (executionContext._replanContext) {
            prompt += `\n---\n${executionContext._replanContext}\n---\n`;
        }

        // Add EDIFACT analysis context if available
        const analysis = executionContext.messageAnalysis;
        if (analysis?.llmContext) {
            prompt += `\n${analysis.llmContext}`;
        } else if (this._looksLikeStructuredData(userMessage)) {
            // No deterministic analysis available — guide the LLM to use createEdiAnalysis
            prompt += `\nNote: No deterministic analysis was performed for this message. ` +
                `If the content appears to be structured data (HL7, NCPDP, TRADACOMS, VDA, or any EDI-like format), ` +
                `use the "createEdiAnalysis" tool to build a structured analysis. ` +
                `Analyze the raw content yourself and fill in the analysis schema.`;
        }
    ...
    /**
     * Heuristic check whether the user message looks like structured/EDI data.
     * Uses simple signals: multi-line, pipe/tilde delimiters, record-like patterns.
     * @param {string} message
     * @returns {boolean}
     * @private
     */
    _looksLikeStructuredData(message) {
        if (!message || message.length < 50) return false;
        const lines = message.split('\n').filter(l => l.trim().length > 0);
        if (lines.length < 3) return false;
        // Check for common delimiters: pipe, tilde, caret, asterisk, colon-heavy
        const delimiterHeavy = (message.match(/[|~^*]/g) || []).length > 10;
        // Check for segment-like patterns (3-char uppercase prefix)
        const segmentLike = lines.filter(l => /^[A-Z0-9]{2,4}[|*+:\t]/.test(l.trim())).length >= 2;
        return delimiterHeavy || segmentLike;
    }
